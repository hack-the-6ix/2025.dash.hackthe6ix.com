...
const downloadIOSPass = async (user: {
  userId: string;
  userType: string;
  userName: string;
}) => {
  ...
  try {
    const response = await fetch(
      `${baseUrl}/passes/apple/hackathon.pkpass?userId=${userId}&userType=${userType}&userName=${userName}`,
      {
        method: "GET",
        headers: {
...
const downloadGooglePass = async (user: {
  userId: string;
  userType: string;
  userName: string;
}) => {
  ...
  try {
    const response = await fetch(
      `${baseUrl}/passes/google/hackathon.pkpass?userId=${userId}&userType=${userType}&userName=${userName}`,
      {
        method: "GET",
        headers: {
          "ngrok-skip-browser-warning": "true"
...
export default function DownloadPass() {
  ...
  }, [userId, userName, userType, navigate]);
